...
class NoteDetailActivity : AppCompatActivity() {
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tagAdder.setAdapter(adapter)
        tagAdder.threshold = 1
        tagAdder.onItemClickListener = AdapterView.OnItemClickListener{
                parent, _, position, id ->
            val tag:Tag = parent.adapter.getItem(position) as Tag
            putChip(tag.name)
            detailViewModel.addTag(tag)
            tagAdder.text.clear()
        }

        tagAdder.onFocusChangeListener = View.OnFocusChangeListener{
                view, b ->
            if(b){
